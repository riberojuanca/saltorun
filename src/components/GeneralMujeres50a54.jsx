...
async function GeneralMujeres50a54() {
  const csv = await fetch(
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vSpScqCy69yANyJrX2PqM2jz1SIGBzHbTU6QzHMqDL-66ypVBlkVxonN-LugpLGLtYE88zwiC2VVoSj/pub?output=tsv"
  ).then((res) => res.text());

  const generalMujeres50a54 = csv
    .split("\n")
    .slice(1)
    .map((row) => {
      const [
  ...
  generalMujeres50a54.sort((a, b) => b.totalPoints - a.totalPoints);
    ...
    <main className="ms-auto me-auto max-w-5xl mt-8 mb-8">
      <section className="p-4 ms-auto me-auto">
        <div className=" flex items-center justify-between text-center px-4 w-fit text-lg md:text-xl gap-4 me-auto ms-auto">
          <h1 className="md:text-2xl font-semibold">GENERAL MUJERES 50 A 54</h1>
        </div>
        <Table className="landscape:hidden md:hidden">
          ...
          <TableBody>
            {generalMujeres50a54.map((player, index) => (
...
export default GeneralMujeres50a54;
