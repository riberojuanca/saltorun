...
async function GeneralHombres20a24() {
  const csv = await fetch(
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vTZffJeO145T4WmSOwRdq8PE83fjO3ymQEtC3f7D5T-BQwqlbg1gel9IR27NhoNy4mcx0R9JuJ_Fl30/pub?output=tsv"
  ).then((res) => res.text());

  const generalHombres20a24 = csv
    .split("\n")
    .slice(1)
    .map((row) => {
  ...
  generalHombres20a24.sort((a, b) => b.totalPoints - a.totalPoints);

  return (
    <section className="p-4 ms-auto me-auto">
      <h1 className="text-2xl">POSICIONES</h1>
      <h2 className="text-xl">GENERAL HOMBRES 20 a 24</h2>
      <Table>
        <TableCaption>
          Posiciones tabla general hombres de 20 a 24 años
        </TableCaption>
        <TableHeader>
          <TableRow>
            <TableHead></TableHead>
        ...
        <TableBody>
          {generalHombres20a24.map((player, index) => (
...
export default GeneralHombres20a24;
