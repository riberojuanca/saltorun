...
async function GeneralMujeres35a39() {
  const csv = await fetch(
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vSlHRQrALwLWK_beml5-eQ37C7onwrmumCQ9JkvycmaPUvHiXkVXXC9NSHCrzZVV4nOv-eB8TgmSuZx/pub?output=tsv"
  ).then((res) => res.text());

  const generalMujeres35a39 = csv
    .split("\n")
    .slice(1)
    .map((row) => {
      const [
  ...
  generalMujeres35a39.sort((a, b) => b.totalPoints - a.totalPoints);
    ...
    <main className="ms-auto me-auto max-w-5xl mt-8 mb-8">
      <section className="p-4 ms-auto me-auto">
        <div className=" flex items-center justify-between text-center px-4 w-fit text-lg md:text-xl gap-4 me-auto ms-auto">
          <h1 className="md:text-2xl font-semibold">GENERAL MUJERES 35 A 39</h1>
        </div>
        <Table className="md:hidden">
          ...
          <TableBody>
            {generalMujeres35a39.map((player, index) => (
...
export default GeneralMujeres35a39;
