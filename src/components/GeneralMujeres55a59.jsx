...
async function GeneralMujeres55a59() {
  const csv = await fetch(
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vSybbrOnrLMYm5wFczQEu2QbXwd8Arspwmb-5SmFoeFELiSOkxmtubxpYj8TU1McGe2XpCyUUhP_5jt/pub?output=tsv"
  ).then((res) => res.text());

  const generalMujeres55a59 = csv
    .split("\n")
    .slice(1)
    .map((row) => {
      const [
  ...
  generalMujeres55a59.sort((a, b) => b.totalPoints - a.totalPoints);
    ...
    <main className="ms-auto me-auto max-w-5xl mt-8 mb-8">
      <section className="p-4 ms-auto me-auto">
        <div className=" flex items-center justify-between text-center px-4 w-fit text-lg md:text-xl gap-4 me-auto ms-auto">
          <h1 className="md:text-2xl font-semibold">GENERAL MUJERES 55 A 59</h1>
        </div>
        <Table className="landscape:hidden md:hidden">
          ...
          <TableBody>
            {generalMujeres55a59.map((player, index) => (
...
export default GeneralMujeres55a59;
